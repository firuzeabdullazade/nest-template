...
export class ArticleService {
  ...
  getArticleById(id: number): ArticlesGridItemProps {
    return articles.find((article) => article.postId === id);
  }

  createArticle(request: CreateArticleRequest): number {
    const newId = articles[articles.length - 1].postId + 1;
    const newArticle: ArticlesGridItemProps = {
      postId: newId,
      date: request.date,
      tag: request.tag,
      imageSrc: request.imageSrc,
      firstPostImage: request.firstPostImage,
      secondPostImage: request.secondPostImage,
  ...
  updateArticle(id: number, request: UpdateArticleRequest) {
    const articleToChange = this.getArticleById(id);
    articleToChange.content = request.content;
    articleToChange.date = request.date;
    articleToChange.firstPostImage = request.firstPostImage;
    articleToChange.imageSrc = request.imageSrc;
    articleToChange.postContent = request.postContent;
    articleToChange.secondPostImage = request.secondPostImage;
    articleToChange.tag = request.tag;
  }

  deleteArticle(id: number) {
    articles = articles.filter((article) => article.postId !== id);
  }
}
